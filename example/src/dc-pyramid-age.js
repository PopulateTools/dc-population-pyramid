...
export default function (parent, chartGroup) {
  ...
  var _leftChartWrapper = d3
    .select(parent)
    .append("div")
    .attr("class", "left-chart");
  var _rightChartWrapper = d3
    .select(parent)
    .append("div")
    .attr("class", "right-chart");
  ...
  **/
  ...
  if (_leftChart.calculateAxisScaleData) {
    ...
    _chart.group = function (_) {
      ...
      _leftChart.x(
        d3.scaleLinear().domain(extent).range([_leftChart.effectiveWidth(), 0])
      );
      _rightChart.x(
        d3.scaleLinear().domain(extent).range([0, _rightChart.effectiveWidth()])
      );
  ...
  function pyramidChart() {
    setTimeout(() => {
      ...
      const leftChartClassName = leftChartId.classList.contains(
        "init-left-chart"
      );
